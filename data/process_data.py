...
def clean_data(df):
    ...
    # Create based on this row a Series of column names except the -0, -1 or -2 part of the string
    category_colnames = pd.Series(row).apply(lambda x: x[:-2])
    
    # rename the columns of `categories`
    categories.columns = category_colnames
    
    #Iterate over each column in categories and set the values equal to the attached value of the string (the last one)
...
def save_data(df, database_filename):
    ...
    #create engine with databse_filename
    engine = create_engine('sqlite:///' + database_filename)
    
    #Save cleaned data in the database within the table "MyDataset"
    df.to_sql('MyDataset', engine, index=False, if_exists='replace') 
